...
<html>
  ...
  <body style="background:url(<%=request.getContextPath() %>/img/4.jpg) no-repeat;background-size:cover;" onload="start()">
  	<div class="admin">
  		...
  		<div class="admin-body">
  			...
  			<div class="admin-content">
  				...
  				<center>
			    	...
			    	<legend>登陆日志表</legend>
			     	<table border="1">
			    	<tr><th>登陆账号</th><th>登陆时间</th><th>退出时间</th></tr>
			    	...
			    	<td colspan="1">
			  			<c:choose>
			  			<c:when test="${page>1}"><a href="logDate?page=${page-1}">上一页</a></c:when>
			  			<c:otherwise><a href="logDate?page=${page}">上一页</a></c:otherwise>
			  			</c:choose>
			  		</td>
			   		<td>
			   			当前页${page}/${pages}
			   			<form action="logDate" method="post"> 
			   			到第 <input  type="text" name="page" size="3" />页 <input type="submit" value="跳转"/>
			   			</form>
			   		</td>
			   		<td colspan="1">
			  			<c:choose>
			  			<c:when test="${page<pages}"><a href="logDate?page=${page+1}">下一页</a></c:when>
			  			<c:otherwise><a href="logDate?page=${page}">下一页</a></c:otherwise>
			  			</c:choose>
			  		</td>
			  		</tr>
			    	</table>
  				...
  				<div id="saveTeacherMessage">
    				...
    				<s:textfield name="teacher.nomber" label="教师工号" size="30" maxlength="6"/>
    				<s:textfield name="teacher.name" label="教师姓名" size="30" maxlength="16"/>
  				...
  				<div id="createTeacherAccount">
    				...
    				<legend>教师账号添加</legend>
     					<form action="admin/createTeaAccount" method="post">
