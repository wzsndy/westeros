  ...
  <div class="admin">
  		...
  		<div class="student-page">
	    	...
	    	<div class="student-content">
	    		...
	    		<tr><td colspan=11 class="t-head" align="center" style="font-weight: 600">院长信息</td></tr>
	    		<tr>
	  				<th>ID</th>
	  				<th>院长编号</th>
	  				<th>院长姓名</th>
	  			...
	  			<c:forEach items="${deans}" var="teacher">
	  			<tr align="center">
	  				<td>${teacher.id}</td>
	  				<td>${teacher.nom}</td>
	  				<td>${teacher.name}</td>
	  				...
	  				<td>${teacher.college.text}</td>
	  			<td>
	  			<c:forEach items="${majors}" var="major">
	  				<c:if test="${teacher.college.id==major.collegeId}">
	  				<p>${major.name}</p>
	  				</c:if>
	  			</c:forEach>
	  			</td>
	  			<td>
	  				<c:if test="${teacher.nom==accountNom}">
	  					<a href="updateTeacher?id=${teacher.id}&accountNom=${teacher.nom}">修改基本信息</a>
	   				</c:if>
	  			</td>
	  			</tr>
	  			</c:forEach>
